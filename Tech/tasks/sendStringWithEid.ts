...
// Task to send a string cross‐chain using MyOApp contract
// Example: 
//      npx hardhat lz:oapp:send --dst-eid 40232 --string "Hello Optimism from Arbitrum" --network arbitrum-testnet
// The above command sends a string from Arbitrum Sepolia to Optimism Sepolia.
    ...
    .setAction(async (args: { dstEid: number; string: string; options?: string }, hre: HardhatRuntimeEnvironment) => {
        ...
        try {
            ...
            DebugLogger.printErrorAndFixSuggestion(
                ...
                `Destination network: ${endpointIdToNetwork(args.dstEid)}, source contract: ${contractAddress} on source network: ${hre.network.name}`
            )
            throw error
        }
