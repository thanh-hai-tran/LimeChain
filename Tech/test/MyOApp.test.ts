...
// Therea are two test cases: Test 1 and Test 2.
// 1. Test sending a string message from one MyOApp instance to another, who know each other as peers.
//    This test case is expected to succeed, as the receiving MyOApp instance should update its lastMessage state variable.
// 2. Test sending a string message from one MyOApp instance to another, who do not know each other as peers.
//    This test case is expected to fail, as the receiving MyOApp instance should not update its lastMessage state variable
//    due to the unknown sender.
...
describe('Test 2', function () {
    ...
    beforeEach(async function () {
        ...
        // Only set myOAppB as a peer of myOAppA,
        // so myOAppB does not know myOAppA as a peer.
        await myOAppA.connect(ownerA).setPeer(eidB, ethers.utils.zeroPad(myOAppB.address, 32))
        // await myOAppB.connect(ownerB).setPeer(eidA, ethers.utils.zeroPad(myOAppA.address, 32))
    })

    // A test case to verify message sending functionality
    it('should not update lastMessage due to an unknown sender', async function () {
        // Assert initial state of lastMessage in both MyOApp instances
        expect(await myOAppA.lastMessage()).to.equal('')
        expect(await myOAppB.lastMessage()).to.equal('')
        ...
        // Assert the resulting state of lastMessage in both MyOApp instances
        expect(await myOAppA.lastMessage()).to.equal('')
        // myOAppB should not update its lastMessage because it does not know myOAppA as a peer.
        // Therefore, it should remain empty.
        expect(await myOAppB.lastMessage()).to.equal('')
    })
})
