...
// MyOApp is a simple OApp that sends and receives strings across chains.
// It extends OApp for LayerZero messaging and OAppOptionsType3 for execution options.
contract MyOApp is OApp, OAppOptionsType3 {
    ...
    // Store processed message GUIDs to prevent replay
    ...
    //
    // Example: send a simple string to a remote chain. 
    // ──────────────────────────────────────────────────────────────────────────────
    ...
    function sendString(uint32 _dstEid, string calldata _string, bytes calldata _options) external payable {
        // 1. Encode any data structures you wish to send into bytes
        //    You can use abi.encode, abi.encodePacked, or directly splice bytes
        //    if you know the format of your data structures
        bytes memory _message = abi.encode(_string);

        // 2. Call OAppSender._lzSend to package and dispatch the cross-chain message
